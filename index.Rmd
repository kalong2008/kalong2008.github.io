...
```{r Regression_1, results='asis', echo = FALSE}
fit <- lm(religion_scale_1 ~ religion_scale_2_r_attend + religion_scale_3_r_pray + religion_scale_4_r_study_r_each_day + living_with_family + household_size + age_r
                     + gender + edu + marital + private_housing
                     , data=dataset2)
tbl_regression(fit, intercept=TRUE, estimate_fun= ~style_pvalue(.x, digits = 2), pvalue_fun = ~style_pvalue(.x, digits = 3), include= c("religion_scale_2_r_attend", "religion_scale_3_r_pray", "religion_scale_4_r_study_r_each_day")) %>% bold_p(t = 0.05) %>% bold_labels() %>% add_glance_table(include = c(r.squared)) %>% as_flex_table()
```
